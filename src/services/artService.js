...
//   if (!response.ok) {
//     throw new Error(
//       `Something has gone wrong at at HTTP status code ${response.status}`
//     );
//   } else {
...
export const getExhibitionCards = (exhibitionIds) => {
  const fetchPromises = exhibitionIds.map((id) =>
    fetch(
      `https://api.artic.edu/api/v1/artworks/${id}?fields=id,title,image_id,artist_title,alt_text,short_description`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Error fetching data for ID ${id} on HTTP status code ${response.status}`
          );
        }
...
export const getExhibitionInfo = async (id) => {
  return fetch(
    `https://api.artic.edu/api/v1/artworks/${id}?fields=id,title,image_id,artist_title,description,short_description,place_of_origin,date_display,style_title`
  ).then((response) => {
    if (!response.ok) {
